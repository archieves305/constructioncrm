...
import { phaseInputSchema } from "@/lib/validators/workflow";
import { templateActor, versioningErrorResponse } from "@/lib/workflows/admin-route";
import { addPhase } from "@/lib/workflows/versioning";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ vid: string }> }) {
  ...
  const parsed = await validateBody(request, phaseInputSchema);
  if (!parsed.ok) return parsed.response;
  try {
    return NextResponse.json(await addPhase(vid, parsed.data, a.user), { status: 201 });
  } catch (err) {
