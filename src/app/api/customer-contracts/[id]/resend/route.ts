...
import { resendContract } from "@/lib/customer-contracts/sign-service";
...
// POST /api/customer-contracts/[id]/resend — a fresh link; the previous one dies.
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const r = await resendContract(id, session.user.id, { to: v.data.to, message: v.data.message, replyTo: session.user.email });
    return NextResponse.json({ ok: true, emailed: r.emailed, signUrl: r.signUrl, sentTo: r.sentTo, expiresAt: r.expiresAt });
  } catch (err) {
    return contractErrorResponse(err, "contracts.resend");
  }
}
