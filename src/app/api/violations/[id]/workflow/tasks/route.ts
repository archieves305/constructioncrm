...
/** A case-specific task inside a workflow phase — no template key, so reconciliation never touches it. */
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canCoordinateWorkflow(user, toJobScope(gate.scope))) return forbidden();
  ...
  const openDeps = depIds.length > 0 ? await prisma.task.count({ where: { id: { in: depIds }, status: { in: ["PENDING", "IN_PROGRESS", "BLOCKED"] } } }) : 0;
  ...
  if (depIds.length > 0) {
    await prisma.taskDependency.createMany({ data: depIds.map((dependsOnTaskId) => ({ taskId: task.id, dependsOnTaskId, kind: "BLOCKING" as const, source: "manual", createdByUserId: user.id })), skipDuplicates: true });
    await recordTaskEvents({ taskId: task.id, actorUserId: user.id, events: depIds.map((d) => ({ type: "DEPENDENCY_ADDED" as const, fromValue: null, toValue: d })) });
  }
