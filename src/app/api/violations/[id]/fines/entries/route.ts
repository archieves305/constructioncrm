...
import { fineEntrySchema } from "@/lib/validators/violation";
import { recordFineEntry } from "@/lib/violations/fine-ledger";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = await validateBody(request, fineEntrySchema);
  if (!parsed.ok) return parsed.response;
  const gate = await requireCase(id, session.user, (p) => p.canManageFines);
  if ("response" in gate) return gate.response;
  try {
    await recordFineEntry(id, parsed.data, session.user);
